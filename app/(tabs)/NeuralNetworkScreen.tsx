...
export default function NeuralNetworkScreen() {
  ...
  const descargarDataset = async () => {
    try {
      ...
      Alert.alert("Descargando", "Comprimiendo y descargando el dataset...");

      const download = await FileSystem.File.downloadFileAsync(
        URL,
        new FileSystem.File(FileSystem.Paths.document, tempFileName)
      );
      if (download) {
        if (await Sharing.isAvailableAsync()) {
          await Sharing.shareAsync(FileSystem.Paths.document.uri + `/${tempFileName}`, {
            mimeType: 'applicaction/zip',
            dialogTitle: 'Guardar dataset'
          })
        }
      }
    } catch (error) {
      console.error(error);
      Alert.alert("Error", "No se pudo descargar el dataset.");
    }
  };
  ...
  const descargarModelo = async () => {
    try {
      ...
      Alert.alert("Descargando", "Descargando modelo .keras...");
  ...
  return (
    ...
    <ThemedView style={styles.container}>
      ...
      <View style={styles.buttonList}>
        ...
        <Button title="Descargar dataset" onPress={descargarDataset} />
        <Button title="Descargar modelo" onPress={descargarModelo} />
      </View>
    </ThemedView>
    </SafeAreaView>
  );
