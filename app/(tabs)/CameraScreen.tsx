...
export default function CameraScreen() {
  ...
  const [classQuantity, setClassQuantity] = useState<{[key: string]: number}>({});
    ...
    useCallback(() => {
      const fetchCounts = async () => {
        ...
        try {
          ...
          setClassQuantity(data); 
        } catch (error) {
          console.log("No se pudo conectar para obtener contadores al volver:", error);
        }
      };
  ...
  const sendPhotoToTrain = async (photoUri: string, label: string) => {
    ...
    try {
      console.log(`Enviando foto a ${URL} con la etiqueta: ${label}`);
    ...
    if (cameraRef.current) {
      ...
      if (selectedCategory) {
        ...
        if (!success) {
          ...
          setClassQuantity(prevCounts => {
            const newCounts = { ...prevCounts };
            const currentVal = newCounts[selectedCategory] || 0;
            newCounts[selectedCategory] = currentVal + 1;
            return newCounts;
          });
        }
      } else {
  ...
  const addCategory = () => {
    ...
    const newCategoryName = findUniqueCategoryName(categories);
  ...
  const findUniqueCategoryName = (existingCategories: { label: string }[]): string => {
    const existingLabels = existingCategories.map(c => c.label);
    let baseName = "Cosa";
    let newLabel = baseName+" 1";
    let counter = 1;
  ...
  const handleNameSave = () => {
    ...
    const normalizedNewLabel = tempName.trim();
    ...
    if (existingLabels.includes(normalizedNewLabel)) {
        Alert.alert("Error de categoría repetida", `Ya existe una categoría llamada "${normalizedNewLabel}". Por favor, elige otro nombre.`);
        return;
    }
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          text: "Eliminar",
          onPress: async () => {
            await deleteCategoryOnServer(categoryName);
  ...
  const deleteCategoryOnServer = async (categoryLabel: string) => {
    ...
    if (!response.ok) {
        throw new Error(result.error || result.message || "Fallo en la eliminación del servidor.");
    }
              ...
              {editingIndex === index ? (
                ...
                <>
                  ...
                  <ThemedText style={styles.categoryQuantity}>
                    {(classQuantity[category.label] || 0) > 0
                      ? ` ${classQuantity[category.label]}` 
                      : '0' 
                    }
                  </ThemedText>
